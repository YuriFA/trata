...
- **Multi-currency direction** (decided 2026-08-28) - implemented by change
  `multi-currency` with ADR-0008. Two refinements the implementation made
  against the recorded wording: planned payments needed no currency field (a
  plan references exactly one account and inherits its currency), and no
  stored rate column exists (a transfer's two amounts pin the rate).